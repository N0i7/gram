/* FLOß Grammatik – "Wortschatz-Kompakt"-Seite
   Rendert alle Karten aus wortschatz-daten.js
   (window.WORTSCHATZ_META / window.WORTSCHATZ_TABLES). Gleiche Logik
   wie unnuetzes-wissen.js / lerntipps.js, eigene Datenquelle. */
(function(){
  const META = window.WORTSCHATZ_META || [];
  ...
  META.forEach(m=>{
    ...
    html += '<section class="topic" id="'+m.id+'"><h2>'+m.title+'</h2><p class="ten">'+m.titleEn+'</p>'+
      '<div class="card">'+(window.WORTSCHATZ_TABLES[m.id]||'')+'</div></section>';
  });
  root.innerHTML = html;
