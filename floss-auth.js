...
(function () {
  ...
  function kontoOeffnen() {
    ...
    c.innerHTML =
      ...
      '<div id="floss-badges" class="floss-badges"></div>' +
      '<div class="floss-konto-aktionen">' +
        (p.ist_lehrer ? '<a class="floss-ghost floss-lehrer-link" href="lehrer.html">Lehrer-Dashboard öffnen</a>' : '') +
  ...
  function statistikLaden() {
    ...
    Promise.all([
      ...
      FLOSS.db.from("test_durchlauf")
        .select("id", { count: "exact", head: true })
        .eq("user_id", FLOSS.user.id).eq("bestanden", true)
    ]).then(function (r) {
      ...
      var testsBestanden = (r[3] && r[3].count) || 0;
      ...
      // Additiver Hook für badges.js — hier bewusst nur ein Event, keine
      // direkte Abhängigkeit, damit floss-auth.js auch ohne badges.js läuft.
      try {
        window.dispatchEvent(new CustomEvent("floss:stats", {
          detail: { ges: ges, quote: quote, streak: streak, testsBestanden: testsBestanden }
        }));
      } catch (e) { /* CustomEvent fehlt in sehr alten Browsern — Badges sind ein Bonus */ }
    }).catch(function () {
