...
## Schritt 10 — KI-Tutor einrichten (10 Minuten)

Neue Seite `tutor.html`: Schüler können eine eigene Frage stellen (statt nur
Stichwörter zu suchen wie bei der bestehenden Suche). Läuft **nur für
angemeldete Nutzer**, gleiches Prinzip wie die Schreiben-Korrektur.

Du brauchst dafür **keinen neuen Gemini-Key** — der aus Schritt 9.1 wird
wiederverwendet. Nur eine zweite Edge Function und eine Migration:

1. Falls noch nicht geschehen: Inhalt von `supabase-migration-neue-funktionen.sql`
   im SQL Editor laufen lassen (**Run**).
2. Supabase Dashboard → **Edge Functions** → **Create a new function**.
3. Name: `tutor` (genau so, klein geschrieben).
4. Inhalt von `supabase/functions/tutor/index.ts` einfügen → **Deploy**.
5. Kein zusätzliches Secret nötig — `GEMINI_API_KEY` aus Schritt 9.4 gilt für
   beide Funktionen.
6. Testen: als Schüler-Account anmelden, auf `tutor.html` eine Frage stellen
   (z.B. „Wann benutze ich Perfekt statt Präteritum?“). Kommt keine Antwort,
   Schritt 10.2–10.4 prüfen. Ohne Deploy zeigt die Seite eine klare
   Fehlermeldung statt kaputt zu wirken.

Tageslimit: 20 Fragen pro Schüler und Tag (in der Edge Function, Konstante
`TAGES_LIMIT` in `supabase/functions/tutor/index.ts`).

## Schritt 11 — Sprechen: Storage-Bucket einrichten (10 Minuten)

Neue Seite `sprechen.html`: Schüler nehmen im Browser eine Antwort zu einem
Prompt auf, du hörst sie dir im Lehrer-Dashboard (`lehrer.html`) an. Bewusst
ohne automatische Bewertung.

1. Falls noch nicht geschehen: `supabase-migration-neue-funktionen.sql`
   ausführen (siehe Schritt 10.1 — dieselbe Datei legt auch die Sprechen-Tabelle an).
2. Supabase Dashboard → **Storage** → **New bucket**.
3. Name: `sprechen` (genau so). **Public bucket** NICHT anhaken — die
   Aufnahmen sind privat, die Seite arbeitet mit zeitlich begrenzten
   signierten Links.
4. Im neuen Bucket → **Policies** → **New policy** → drei Policies anlegen
   (am einfachsten über „For full customization“ mit eigenem SQL):

   ```sql
   -- Schüler dürfen nur in ihren eigenen Ordner hochladen (Pfad beginnt mit ihrer user_id)
   create policy "eigene aufnahme hochladen"
   on storage.objects for insert to authenticated
   with check (bucket_id = 'sprechen' and (storage.foldername(name))[1] = auth.uid()::text);

   -- Schüler dürfen ihre eigenen Aufnahmen lesen (für die Vorschau)
   create policy "eigene aufnahme lesen"
   on storage.objects for select to authenticated
   using (bucket_id = 'sprechen' and (storage.foldername(name))[1] = auth.uid()::text);

   -- Lehrer dürfen alle Aufnahmen lesen (fürs Dashboard)
   create policy "lehrer liest alle aufnahmen"
   on storage.objects for select to authenticated
   using (bucket_id = 'sprechen' and public.ist_lehrer());
   ```

5. Testen: als Schüler-Account auf `sprechen.html` einen Prompt aufnehmen und
   hochladen (Mikrofon-Erlaubnis im Browser bestätigen). Danach als
...
- **`wiederholen.html`** — liest nur die bereits bestehende Tabelle
  `uebung_fortschritt`, keine neue Tabelle, kein Secret.
- **`lehrer.html`** — nutzt die „lehrer liest …“-Regeln, die schon in
  `supabase-schema.sql` stehen. Voraussetzung ist nur Schritt 6 (dich selbst
  zum Lehrer machen).
- **Badges im Konto-Bereich** (`badges.js`) und die **Offline-Fähigkeit**
  (`sw.js` — Übungen laufen bei schlechter Verbindung weiter) — rein
  clientseitig, keine Datenbank beteiligt.
...
| `tutor.html` / `tutor.js` / `tutor.css` | **Neu (21.08.2026).** KI-Tutor: freie Chat-Fragen statt nur Stichwort-Suche |
| `supabase/functions/tutor/index.ts` | **Neu.** Edge Function für den Tutor, gleiches Muster wie `korrektur` |
| `lehrer.html` / `lehrer.js` / `lehrer.css` | **Neu.** Lehrer-Dashboard: Schülerliste, Aktivität, Fortschritt, offene Sprechen-Aufnahmen |
| `wiederholen.html` / `wiederholen.js` / `wiederholen.css` | **Neu.** Adaptiver Wiederholungsmodus — zeigt die schwächsten Themen pro Schüler |
| `sprechen.html` / `sprechen.js` / `sprechen.css` | **Neu.** Sprechen-Grundgerüst: Browser-Aufnahme, Upload in Storage, Durchsicht im Lehrer-Dashboard |
| `supabase-migration-neue-funktionen.sql` | **Neu.** Tabellen für Tutor und Sprechen — siehe Schritt 10/11 |
| `sw.js` / `sw-register.js` | **Neu.** Service Worker für Offline-Fähigkeit, rein clientseitig |
| `badges.js` | **Neu.** Gamification-Badges im Konto-Bereich, rein clientseitig |
...
**Zusätzlich geändert am 21.08.2026:** alle Haupt-HTML-Seiten (neue
Nav-Punkte „Wiederholen“ und „Tutor“, `sw-register.js`- und `badges.js`-
Einbindung, additiv, keine bestehende Zeile entfernt), `floss-auth.js`
(Badges-Platzhalter im Konto-Dialog, Lehrer-Dashboard-Link für
`ist_lehrer`-Nutzer, ein zusätzliches `floss:stats`-Event) und
`floss-auth.css` (Stile dafür). `modelltests.html` hat zusätzlich einen
Hinweis-Link zu `sprechen.html` bekommen.
